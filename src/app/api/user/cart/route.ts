 ...
 * Uses the priceAtAdd field to calculate totals, ensuring deal prices are respected.
 */
  ...
  try {
    ...
    if (!userID) {
      return NextResponse.json(
        { error: "Error getting user data from token: jwt must be provided" },
        { status: 401 }
      );
    }
    ...
    const cart = await Cart.findOne({ user: userID }).populate({
      path: "items.product",
      select: "_id name price images productType sizes title coverIMG category",
    });

    if (!cart || cart.items.length === 0) {
      return NextResponse.json(
        { totalQuantity: 0, totalPrice: 0, items: [], totalSavings: 0 },
        { status: 200 }
      );
    }
    ...
    // Loop through each cart item to compute totals using priceAtAdd
    for (const item of cart.items) {
      // Use the stored priceAtAdd which already accounts for any deals
      ...
      totalOriginalPrice = currencyOperations.add(
        totalOriginalPrice,
        currencyOperations.multiply(originalItemPrice, item.quantity)
      );
