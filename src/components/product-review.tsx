...
const ProductReviewComponent = ({
  ...
  return (
    <Card className="border shadow-xs rounded-lg" ref={elementRef}>
...
export const ProductReviewComponent2 = ({
  item,
}: {
  item: { _id: string; productType: string };
}) => {
  const elementRef = useRef<HTMLDivElement | null>(null);
  const [hasBeenVisible, setHasBeenVisible] = useState(false);
  const [productReviews, setProductReviews] = useState<ProductReviews[] | null>(
  ...
  useEffect(() => {
    ...
    fetchProductReviews();
  }, [hasBeenVisible, currentPage, item._id]);

  return (
    <div className="px-2">
      <div className="-mt-6 mb-4 flex flex-col md:flex-row gap-4 md:items-center md:justify-between">
        <div className="flex items-center gap-2">
          <div className="text-3xl font-bold">{averageRating.toFixed(1)}</div>
          <div className="flex flex-col">
            ...
            <div key={index} className="flex items-center gap-1">
              ...
              <div className="h-1.5 w-16 bg-gray-200 rounded-full overflow-hidden">
                <div
                  className="h-full bg-yellow-400"
                  style={{
                    width: `${(item.count / reviewCounts.totalReviews) * 100}%`,
                  }}
                />
              </div>
            </div>
