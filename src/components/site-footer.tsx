      ...
      <div className="mx-auto max-w-7xl overflow-hidden px-6 py-12 sm:py-14 lg:px-8 flex flex-col gap-6 items-center">
        ...
        >
          {siteConfig.footer.map((item) => (
            <div key={item.name} className="pb-6">
              ...
              <Link href={item.href} className="text-sm leading-6 dark:hover:bg-transparent dark:hover:text-udua-orange-primary hover:bg-transparent hover:text-udua-orange-primary delay-75 transition-all ease-in-out">
                {item.name}
              </Link>
        ...
        <section>
          ...
          {siteConfig.paymentmethods.map(method => (
            <Image src={method.img} height={50} width={50} alt={method.img} key={method.img}/>
          ))}
          </div>
        </section>
